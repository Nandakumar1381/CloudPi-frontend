...
export default function CardComponent() {
  ...
  const handleDeleteConfirm = () => {
    if (selectedFileId) {
      fetch(`http://127.0.0.1:8000/music/${selectedFileId}/delete/`, {
        method: 'DELETE',
          ...
          <Grid item xs={12} sm={6} md={4} key={file.id}>
            <Card sx={{ width: 250, height: 250 }} style={{ border: '1px solid #e0e0e0' }}>
              <CardHeader
                ...
                title={truncateFileName(file.title)}
                subheader={file.date}
              />
              <CardMedia 
                component={PlayCircleOutlineIcon}
                height="194"
                style={{ fontSize: 90, color: '#E3E1FF', marginLeft: 75 }} // Customize the size and color of the icon as needed
                alt="File Preview" />

              <CardContent>{/* Content goes here */}</CardContent>
              <CardActions disableSpacing style={{ position: 'relative' }}>
  <IconButton aria-label="add to favorites" onClick={handleStarClick} style={{  }}>
    <StarBorderOutlinedIcon style={{ color: iconColor }} />
  </IconButton>
  <IconButton aria-label="delete" onClick={() => handleDeleteClick(file.id)} style={{  }}>
